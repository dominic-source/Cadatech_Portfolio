  ...
  return(
    ...
    <Grid container className={styles.goals} p={3}>
      <Grid item xs={12} sm={10} md={6} sx={{borderRight:'2px solid #08d9d6'}}>
        <div >
            <div className={styles.sizee}>If we reach</div>

            <span className={styles.sizek}>£{item.id*9025}</span>


            <div className={`${styles.sizei} ${stylescampaign.pledgecontentadj}`}>{item.title}</div>

            <div className={styles.sizee} style={{minWidth:'235px'}}>{item.content}</div>
        </div>
      ...
      <Grid item xs={12} sm={10} md={6} sx={{position:'relative',mr:{xs:5,sm:0,md:0},pl:{xs:0,sm:2,md:4}}} >
        ...
        <div className={`${stylescampaign.votedesign} ${styles.sizeh}`}>Vote on the design</div>
      </Grid>
    </Grid>
