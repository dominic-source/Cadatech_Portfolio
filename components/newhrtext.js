...
export default function Newhrtext(props){
    ...
    <Typography variant='h5' sx={{mt:3,mb:2}} className={styles.sizei}>{props.text1}</Typography>
    <Typography variant='h6' component="h5" sx={{mt:3,mb:2}} className={styles.sizeh}>{props.text2}</Typography>
    <Typography variant="subtitle2" sx={{mt:2}} className={styles.sizec}>{props.text3}</Typography>
  </Box>
  </>)
  }